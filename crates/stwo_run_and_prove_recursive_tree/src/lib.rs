...
/// (TrainProved.proof, TrainProved.fact_topologies[0], TrainProved.flatten_task_outputs).
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct LeafInput {
    /// Leaf train id; carried for logging and for populating `RecursiveJobData` if requested.
    pub train_id: u64,
    ...
    /// One of "pedersen", "poseidon", "blake".
    pub program_hash_function: String,
    pub fact_topology: FactTopology,
    /// Flat task outputs of the leaf bootloader run (= what the verifier reads off the
    /// previous layer). Hex strings, decoded to `Felt252` on load.
...
impl RecursiveJobData {
    ...
    fn from_leaf(leaf: &LeafInput) -> Self {
        Self {
...
pub struct RecursiveTreeConfig {
    ...
    /// Read by Python (`StwoProver._cairo_prove_recursive_tree`) as `train_proved.fact_topologies`.
...
pub fn stwo_run_and_prove_recursive_tree(
    ...
    let mut current_layer: Vec<LayerEntry> = leaves
        ...
        .map(|leaf| LayerEntry {
            ...
            fact_topology: leaf.fact_topology.clone(),
            outputs: leaf.outputs.clone(),
            aggregated: RecursiveJobData::from_leaf(&leaf),
...
fn reduce_pair(
    ...
    let program = get_program(bootloader_program.as_path()).map_err(|e| {
        RecursiveTreeError::Program(e, bootloader_program.clone())
    })?;
    ...
    let new_fact_topologies = read_fact_topologies_file(&fact_topologies_dump_path)?;
    if new_fact_topologies.len() != 1 {
        return Err(RecursiveTreeError::Anyhow(anyhow::anyhow!(
            "Expected exactly one fact topology from a 2-task simple bootloader run, got {}.",
            new_fact_topologies.len()
        )));
    }
    let new_fact_topology = new_fact_topologies.into_iter().next().unwrap();
    ...
    if save_debug_data && let Some(dir) = debug_data_dir {
        ...
        let dump_path =
            dir.join(format!("prover_input_layer_{layer_idx:03}_pair_{pair_idx:03}.json"));
        fs::create_dir_all(dir)?;
        std::fs::write(&dump_path, sonic_rs::to_string_pretty(&prover_input)?)
            .map_err(|e| RecursiveTreeError::PathIO(e, dump_path))?;
    }

    let aggregated = RecursiveJobData::combine(&left.aggregated, &right.aggregated, outputs.clone());
    let packed_output = PackedOutput::Composite(CompositePackedOutput {
        ...
        fact_topologies: vec![left.fact_topology.clone(), right.fact_topology.clone()],
    });
    Ok(LayerEntry {
        proof_path: proof_output_path,
        fact_topology: new_fact_topology,
        outputs,
        aggregated,
        packed_output,
    })
...
fn write_root_outputs(
    ...
    write_to_fact_topologies_file(fact_topologies_path, &[&root.fact_topology])
...
/// Serde helpers for `Vec<Felt252>` <-> JSON `Vec<String>` (decimal/hex). We accept any input
/// hex string Felt252's `from_hex` understands (with or without the `"0x"` prefix, and in
/// particular the `to_hex_string()` round-trip output) and emit decimal strings on serialization
/// so that Python's `marshmallow.fields.Integer` (used by `PackedOutput.outputs`) can parse them
/// without losing precision on big values (Felt252 exceeds JSON's safe integer range).
mod felt_hex_vec {
    use cairo_vm::Felt252;
        ...
        S: Serializer,
    {
        ...
        for value in values {
            // Decimal string preserves Felt252's full 252-bit precision through any JSON parser
            // and is accepted by Python's `marshmallow.fields.Integer.deserialize`.
            seq.serialize_element(&value.to_string())?;
        }
        seq.end()
    }

    pub fn deserialize<'de, D>(deserializer: D) -> Result<Vec<Felt252>, D::Error>
    where
        D: Deserializer<'de>,
    {
        // Accept either string-encoded values (our preferred form) or JSON numbers (for
        // backwards compatibility with consumers that emit small ints directly).
        #[derive(Deserialize)]
        #[serde(untagged)]
        enum FeltLike {
            Hex(String),
        ...
        let raw: Vec<FeltLike> = Vec::deserialize(deserializer)?;
        raw.into_iter()
            .map(|item| match item {
                FeltLike::Hex(s) => {
                    // Try hex first (matches our `to_hex_string()` writes), then decimal.
                    Felt252::from_hex(&s)
                        .or_else(|_| Felt252::from_dec_str(&s))
                        .map_err(serde::de::Error::custom)
                }
                FeltLike::Number(n) => {
                    // serde_json::Number is decimal; route through string for Felt252.
                    Felt252::from_dec_str(&n.to_string()).map_err(serde::de::Error::custom)
                }
            })
            .collect()
    }
