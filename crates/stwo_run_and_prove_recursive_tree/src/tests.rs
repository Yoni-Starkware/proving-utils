...
use super::{LeafInput, RecursiveJobData, RecursiveTreeConfig, RecursiveTreeError, stwo_run_and_prove_recursive_tree};
...
fn aggregated_combine_sums_per_leaf_counters() {
    ...
    let leaf_b = LeafInput {
        ...
        program_hash_function: "blake".to_string(),
        fact_topology: FactTopology {
            tree_structure: vec![1, 0, 1, 0],
            page_sizes: vec![5, 7],
        },
        outputs: vec![Felt252::from(2u64)],
        n_non_recursive_jobs: 1,
        total_non_recursive_output_size: 12,
        total_n_pages: 2,
        total_fact_tree_structures_len: 4,
    };
    let agg_a = RecursiveJobData::from_leaf(&leaf_a);
    let agg_b = RecursiveJobData::from_leaf(&leaf_b);
